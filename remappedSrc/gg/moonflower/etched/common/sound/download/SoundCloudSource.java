...
public class SoundCloudSource implements SoundDownloadSource {
    ...
    private static final Text BRAND = Text.translatable("sound_source." + Etched.MOD_ID + ".sound_cloud").styled(style -> style.withColor(TextColor.fromRgb(0xFF5500)));
    ...
    private <T> T resolve(String url, @Nullable DownloadProgressListener progressListener, Proxy proxy, SourceRequest<T> function) throws IOException, JsonParseException {
        try (InputStreamReader reader = new InputStreamReader(this.get("https://api-v2.soundcloud.com/resolve?url=" + URLEncoder.encode(url, StandardCharsets.UTF_8), progressListener, proxy, 0, true))) {
            ...
            String kind = JsonHelper.getString(json, "kind");
            if (!"track".equals(kind) && !"playlist".equals(kind)) {
                throw new IOException("URL is not a track or album");
            }
            if ("track".equals(kind) && !JsonHelper.getBoolean(json, "streamable")) {
                throw new IOException("URL is not streamable");
            }
            if ("playlist".equals(kind) && !JsonHelper.getBoolean(json, "is_album")) {
                throw new IOException("URL is not a track or album");
            }
    ...
    public List<URL> resolveUrl(String url, @Nullable DownloadProgressListener progressListener, Proxy proxy) throws IOException {
        return this.resolve(url, progressListener, proxy, json -> {
            ...
            JsonArray media = JsonHelper.getArray(JsonHelper.getObject(json, "media"), "transcodings");

            int progressiveIndex = -1;
            for (int i = 0; i < media.size(); i++) {
                JsonObject transcodingJson = JsonHelper.asObject(media.get(i), "transcodings[" + i + "]");
                ...
                if ("hls".equals(protocol)) {
                    try (InputStreamReader urlReader = new InputStreamReader(this.get(JsonHelper.getString(transcodingJson, "url"), null, proxy, 0, true))) {
                        try (InputStreamReader reader = new InputStreamReader(this.get(JsonHelper.getString(JsonParser.parseReader(urlReader).getAsJsonObject(), "url"), null, proxy, 0, false))) {
            ...
            try (InputStreamReader reader = new InputStreamReader(this.get(JsonHelper.getString(JsonHelper.asObject(media.get(progressiveIndex), "transcodings[" + progressiveIndex + "]"), "url"), null, proxy, 0, true))) {
                return Collections.singletonList(new URL(JsonHelper.getString(JsonParser.parseReader(reader).getAsJsonObject(), "url")));
            }
        });
    }
    ...
    public List<TrackData> resolveTracks(String url, @Nullable DownloadProgressListener progressListener, Proxy proxy) throws IOException, JsonParseException {
        return this.resolve(url, progressListener, proxy, json -> {
            JsonObject user = JsonHelper.getObject(json, "user");
            String artist = JsonHelper.getString(user, "username");
            String title = JsonHelper.getString(json, "title");
            String kind = JsonHelper.getString(json, "kind");
            if ("playlist".equals(kind)) {
                JsonArray tracksJson = JsonHelper.getArray(json, "tracks");
                List<TrackData> tracks = new ArrayList<>();
                tracks.add(new TrackData(url, artist, Text.literal(title)));

                for (int i = 0; i < tracksJson.size(); i++) {
                    try {
                        JsonObject trackJson = JsonHelper.asObject(tracksJson.get(i), "tracks[" + i + "]");
                        if (!trackJson.has("permalink_url")) { // Paid song
                            continue;
                        }
                        JsonObject trackUser = JsonHelper.getObject(trackJson, "user", user);
                        String trackUrl = JsonHelper.getString(trackJson, "permalink_url");
                        String trackArtist = JsonHelper.getString(trackUser, "username");
                        String trackTitle = JsonHelper.getString(trackJson, "title");
    ...
    public Optional<String> resolveAlbumCover(String url, @Nullable DownloadProgressListener progressListener, Proxy proxy, ResourceManager resourceManager) throws IOException {
        return this.resolve(url, progressListener, proxy, json -> {
            ...
            return Optional.of(JsonHelper.getString(json, "artwork_url"));
        });
    }
