    ...
    public static NativeImage apply(NativeImage image, NativeImage overlay) {
        ...
        for (int m = 0; m < overlay.getWidth(); ++m) {
            for (int n = 0; n < overlay.getHeight(); ++n) {
                ...
                int baseColor = alphaBlend(image.getColor(x1, y1), image.getColor(x2, y1), image.getColor(x1, y2), image.getColor(x2, y2));
                int overlayColor = overlay.getColor(m, n);
                nativeImage2.setColor(m, n, (overlayColor & 0xFF000000) | multiply(baseColor, overlayColor, 16) | multiply(baseColor, overlayColor, 8) | multiply(baseColor, overlayColor, 0));
            }
        }
