...
public class StreamingAudioSource implements AudioSource {
    ...
    public StreamingAudioSource(URL[] urls, @Nullable DownloadProgressListener progressListener, boolean temporary, AudioFileType type) {
        ...
        this.downloadFuture = CompletableFuture.allOf(IntStream.range(0, files).mapToObj(i -> CompletableFuture.runAsync(() -> AudioSource.downloadTo(urls[i], temporary, accumulatingListener, type), NetworkUtils.EXECUTOR)).toArray(CompletableFuture[]::new));
    }
    ...
    public CompletableFuture<InputStream> openStream() {
        if (this.stream == null) {
            this.stream = this.downloadFuture.thenApplyAsync(__ -> {
                try {
                    return new StreamingInputStream(this.urls, i -> CompletableFuture.supplyAsync(() -> AudioSource.downloadTo(this.urls[i], this.temporary, null, this.type), NetworkUtils.EXECUTOR).thenApplyAsync(stream -> {
                        try {
                            return stream.get();
                        } catch (Exception e) {
