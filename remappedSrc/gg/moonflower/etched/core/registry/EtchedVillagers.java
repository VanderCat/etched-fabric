...
public class EtchedVillagers {
    ...
    public static final DeferredRegister<PointOfInterestType> POI_REGISTRY = DeferredRegister.create(ForgeRegistries.POI_TYPES, Etched.MOD_ID);
    ...
    public static final RegistryObject<PointOfInterestType> BARD_POI = POI_REGISTRY.register("bard", () -> new PoiType(ImmutableSet.<BlockState>builder().addAll(Blocks.NOTE_BLOCK.getStateDefinition().getPossibleStates()).build(), 1, 1));
    ...
    private static void registerTrades(Function<Integer, TradeRegistry> context) {
        ...
        // sucks to suck forge
        Registries.ITEM.getEntryList(ItemTags.MUSIC_DISCS).ifPresent(tag -> tag.stream().forEach(item -> tier5.add(item.value(), 10, 1, 4, 40, true)));
    }

    @SubscribeEvent
    public static void onEvent(ServerAboutToStartEvent event) {
        DynamicRegistryManager.Immutable access = event.getServer().registryAccess();
        Optional<Registry<StructurePool>> templateRegistryOptional = access.getOptional(RegistryKeys.TEMPLATE_POOL);
        Optional<Registry<StructureProcessorList>> processorListRegistyOptional = access.getOptional(RegistryKeys.PROCESSOR_LIST);

        if (templateRegistryOptional.isEmpty() || processorListRegistyOptional.isEmpty()) {
            return;
        }

        Registry<StructurePool> templatePools = templateRegistryOptional.get();
        Registry<StructureProcessorList> processorLists = processorListRegistyOptional.get();
        createVillagePiece(templatePools, processorLists, "plains", "bard_house", 1, 2, StructureProcessorLists.MOSSIFY_10_PERCENT, StructureProcessorLists.ZOMBIE_PLAINS);
        createVillagePiece(templatePools, processorLists, "desert", "bard_house", 1, 2, StructureProcessorLists.ZOMBIE_DESERT);
        createVillagePiece(templatePools, processorLists, "savanna", "bard_house", 1, 4, StructureProcessorLists.ZOMBIE_SAVANNA);
        createVillagePiece(templatePools, processorLists, "snowy", "bard_house", 1, 4, StructureProcessorLists.ZOMBIE_SNOWY);
        createVillagePiece(templatePools, processorLists, "taiga", "bard_house", 1, 4, StructureProcessorLists.MOSSIFY_10_PERCENT, StructureProcessorLists.ZOMBIE_TAIGA);
    }

    private static void createVillagePiece(Registry<StructurePool> templatePools, Registry<StructureProcessorList> processorLists, String village, String name, int houseId, int weight, RegistryKey<StructureProcessorList> zombieProcessor) {
        createVillagePiece(templatePools, processorLists, village, name, houseId, weight, StructureProcessorLists.EMPTY, zombieProcessor);
    }

    private static void createVillagePiece(Registry<StructurePool> templatePools, Registry<StructureProcessorList> processorLists, String village, String name, int houseId, int weight, RegistryKey<StructureProcessorList> normalProcessor, RegistryKey<StructureProcessorList> zombieProcessor) {
        EtchedVillagers.addToPool(templatePools.get(new Identifier("village/" + village + "/houses")), new Identifier(Etched.MOD_ID, "village/" + village + "/houses/" + village + "_" + name + "_" + houseId), processorLists.getEntry(normalProcessor).orElse(null), weight);
        EtchedVillagers.addToPool(templatePools.get(new Identifier("village/" + village + "/zombie/houses")), new Identifier(Etched.MOD_ID, "village/" + village + "/houses/" + village + "_" + name + "_" + houseId), processorLists.getEntry(zombieProcessor).orElse(null), weight);
    }

    private static void addToPool(@Nullable StructurePool pool, Identifier pieceId, @Nullable RegistryEntry<StructureProcessorList> processorList, int weight) {
        if (pool == null || processorList == null) {
            return;
        }

        StructurePoolElement piece = StructurePoolElement.ofProcessedLegacySingle(pieceId.toString(), processorList).apply(StructurePool.Projection.RIGID);
    ...
    private static class TradeRegistry implements List<TradeOffers.Factory> {
    ...
    private static class ItemTrade implements TradeOffers.Factory {

        private final Supplier<? extends ItemConvertible> item;
        ...
        private ItemTrade(Supplier<? extends ItemConvertible> Item, int emeralds, int itemCount, int maxUses, int xpGain, float priceMultiplier, boolean sellToVillager) {
            this.item = Item;
