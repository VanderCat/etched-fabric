...
public class EtchedClient {
    ...
    public static void init() {
        RegisterAtlasSpriteEvent.event(PlayerScreenHandler.BLOCK_ATLAS_TEXTURE).register((atlas, registry) -> {
        ...
        ModelRegistry.registerSpecial(new ModelIdentifier(new Identifier(Etched.MOD_ID, "boombox_in_hand"), "inventory"));
        ...
        EntityModelLayerRegistry.register(EtchedModelLayers.JUKEBOX_MINECART, MinecartEntityModel::createBodyLayer);
    ...
    public static void postInit() {
        ...
        ItemPropertiesRegistry.register(EtchedItems.BOOMBOX.get(), new Identifier(Etched.MOD_ID, "playing"), (stack, level, entity, i) -> {
            if (!(entity instanceof Player))
                return 0;
            InteractionHand hand = BoomboxItem.getPlayingHand(entity);
            return hand != null && stack == entity.getItemInHand(hand) ? 1 : 0;
        });
        ItemPropertiesRegistry.register(EtchedItems.ETCHED_MUSIC_DISC.get(), new Identifier(Etched.MOD_ID, "pattern"), (stack, level, entity, i) -> EtchedMusicDiscItem.getPattern(stack).ordinal() / 10F);

        RenderTypeRegistry.register(RenderLayer.getCutout(), EtchedBlocks.ETCHING_TABLE.get(), EtchedBlocks.RADIO.get());

        ItemRendererRegistry.registerHandModel(EtchedItems.BOOMBOX.get(), new ModelIdentifier(new Identifier(Etched.MOD_ID, "boombox_in_hand"), "inventory"));
